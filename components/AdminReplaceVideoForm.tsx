...
export function AdminReplaceVideoForm({
  ...
  const [seconds, setSeconds] = useState(String(thumbnailSec));
  ...
  async function handleSave() {
    ...
    if (!Number.isInteger(secondsValue) || secondsValue < 0) {
      setStatus({ type: "error", message: "Invalid thumbnail time" });
      return;
    }
  ...
  const previewSeconds = Number(seconds);
  const previewUrl =
    Number.isInteger(previewSeconds) && previewSeconds > 0
      ? `${thumbnailBaseUrl}?time=${previewSeconds}s`
      : thumbnailBaseUrl;
  ...
  return (
    <div className="mt-2 flex flex-wrap items-start gap-4">
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-1">
          <label className="font-label font-bold text-[11px] uppercase tracking-[0.18em] text-muted">
            Thumbnail time (seconds into video)
          </label>
          <input
            type="number"
            min="0"
            ...
            className="w-24 border border-line bg-canvas px-3 py-1.5 text-[13px] text-primary outline-none focus:border-strong"
          />
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            onClick={() => {
              ...
              setLink(downloadUrl);
              setSeconds(String(thumbnailSec));
              setStatus({ type: "idle" });
            }}
